...
/*   Updated: 2025/08/18 17:32:41 by jdebrull         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_in_3d(t_data *data, int x, int side)
{
	...
	line_height = (int)(SCREEN_HEIGHT / data->rays.perp_wall_dist);
	...
	if (side == 1)
		...
		color = 0x00FF0000;
	draw_vertical_line(data, x, start_point, end_point, color);
}

void	wall_dist(t_data *data, t_rays *rays, int side, int x)
{
	if (side == 0)
		rays->perp_wall_dist = (rays->map_x - data->player.x + (1 - rays->step_x) / 2.0) / rays->raydirX;
	else
		rays->perp_wall_dist = (rays->map_y - data->player.y + (1 - rays->step_y) / 2.0) / rays->raydirY;
	if (rays->perp_wall_dist == 0)
		rays->perp_wall_dist = 0.0001;
	//draw_pov(data, rays);
	draw_in_3d(data, x, side);
}
